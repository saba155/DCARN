...
import ecc
from modules import RNNGraphConvModule, ECC_CRFModule, GRUCellEx, LSTMCellEx, RNN
...
class GraphNetwork(nn.Module):
    ...
    def __init__(self, config, nfeat, fnet_widths, fnet_orthoinit=True, fnet_llbias=True, fnet_bnidx=-1, edge_mem_limit=1e20):
        ...
        for d, conf in enumerate(config.split(',')):
            ...
            if conf[0]=='f':    #Fully connected layer;  args: output_feats
            ...
            elif conf[0]=='gru' or conf[0]=='lstm' or conf[0] == 'resrnn': #RNN-ECC     args: repeats, mv=False, layernorm=True, ingate=True, cat_all=True
                nrepeats = int(conf[1])
                ...
                if conf[0]=='gru':
                    ...
                    cell = RNN(nfeat, nfeat, bias=True, layernorm=layernorm, ingate=ingate) #, bias=True, layernorm=layernorm, ingate=ingate
