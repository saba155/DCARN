...
def get_datasets(args, test_seed_offset=0):
    ...
    trainset = ['train/' + f for f in os.listdir(args.CUSTOM_SET_PATH + '/DCARN/train')]
    testset  = ['test/' + f for f in os.listdir(args.CUSTOM_SET_PATH + '/DCARN/train')]
    
    # Load superpoints graphs
    testlist, trainlist = [], []
    for n in trainset:
        trainlist.append(spg.spg_reader(args, args.CUSTOM_SET_PATH + '/DCARN/' + n + '.h5', True))
    for n in testset:
        testlist.append(spg.spg_reader(args, args.CUSTOM_SET_PATH + '/DCARN/' + n + '.h5', True))
